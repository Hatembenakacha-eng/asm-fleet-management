...
export class AffectationForm implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.voitureService.getAll('disponible')
      .subscribe(res => { this.voitures = res.data; this.cdr.detectChanges(); });

    this.missionService.getAll()
      .subscribe(res => { this.missions = res.data; this.cdr.detectChanges(); });

    this.employeeService.getAll()
      .subscribe(res => { this.employees = res.data; this.cdr.detectChanges(); });
  }
  ...
  onSubmit(): void {
    ...
    this.affectationService.create(payload).subscribe({
      ...
      error: (err) => {
        ...
        this.cdr.detectChanges(); // Force l'affichage immédiat du message d'erreur
      }
    });
  }
